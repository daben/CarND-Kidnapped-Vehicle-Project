...
int main() {
	...
	// Read map data
	Map map;
	if (!read_map_data(ROOT_PATH"data/map_data.txt", map)) {
	...
	if (!read_control_data(ROOT_PATH"data/control_data.txt", position_meas)) {
	...
	if (!read_gt_data(ROOT_PATH"data/gt_data.txt", gt)) {
	...
	for (int i = 0; i < num_time_steps; ++i) {
		...
		ostringstream file;
		file << ROOT_PATH"data/observation/observations_" << setfill('0') << setw(6) << i+1 << ".txt";
